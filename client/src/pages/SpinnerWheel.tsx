...
import { AlertCircle, Plus, Trash2, RotateCw, Users, HelpCircle, CheckCircle2, XCircle, Edit2, X } from "lucide-react";
...
function QuestionCard({ question }: QuestionCardProps) {
  ...
  useEffect(() => { setSelectedIdx(-1); }, [question.id]);
    ...
    <div className="bg-indigo-950 border border-indigo-700 rounded-xl p-4 space-y-3">
      ...
      {answered && (
        <div className={`flex items-center gap-2 px-4 py-3 rounded-lg font-bold text-lg transition-all ${
          ...
          {isCorrectAnswer ? (
            <><CheckCircle2 className="w-6 h-6" /> <span>إجابة صحيحة! 🎉</span></>
          ) : (
            <><XCircle className="w-6 h-6" /> <span>إجابة خاطئة</span></>
          )}
        </div>
      )}
